...
export class UserService {
  ...
  async getUserById(request: GetUserRequestDto): Promise<GetUserResponseDto> {
    ...
    // Get user
    const user = await this.userRepository.findById(Number(getRequest.userId));
    if (!user) {
  ...
  async updateUserById(
    ...
    // Check user
    const userCount = await this.userRepository.countById(Number(Number(updateRequest.userId)));
    if (userCount == 0) {
      this.logger.error(`User with Id ${Number(updateRequest.userId)} not found`);
      throw new HttpException('User not found', 404);
    }
    ...
    if (updateRequest.email) {
      const emailCheck = await this.userRepository.countByEmail(
        Number(Number(updateRequest.userId)),
        updateRequest.email,
      );
      if (emailCheck > 0) {
        this.logger.error(
  ...
  async updateUserAvatar(
    ...
    // Check user
    const user = await this.userRepository.findById(Number(updateRequest.userId));
    if (!user) {
      this.logger.error(`User with Id ${Number(updateRequest.userId)} not found`);
      throw new HttpException('User not found', 404);
    }
    ...
    const updatedUser = await this.userRepository.updateById(user.id, {
      ...
      email: user.email,
      fullname: user.fullname,
      // properti lain yang ingin diupdate
    });
    return {
      id: String(updatedUser.id),
        email: updatedUser.email ?? "",
